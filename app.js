...
const DEFAULT_AVATAR_URL = "./assets/ftc-logo.png?v=20260708-sidebar-logo-attendance";
  ...
  if (nav && !$("#nav-dropdown-toggle")) {
    ...
    nav.innerHTML = `
      ...
      <div class="nav-dropdown-menu">
        ...
        ${navButton("attendance-section", "Absensi", "clipboard-check")}
...
function paintAvatar(el, name, url) {
  if (!el) return;
  const avatar = url || DEFAULT_AVATAR_URL;
  el.textContent = "";
  el.style.setProperty("background-image", `url("${avatar}")`, "important");
  el.style.setProperty("background-size", url ? "cover" : "contain", "important");
  el.style.setProperty("background-position", "center", "important");
  el.style.setProperty("background-repeat", "no-repeat", "important");
  el.style.setProperty("background-color", url ? "transparent" : "#fff", "important");
}
...
function switchSection(sectionId) {
  ...
  $$(".nav-item").forEach((item) => {
    ...
    if (active) { text(els.activeNavLabel, item.dataset.label); els.activeNavIcon?.setAttribute("data-lucide", item.dataset.icon || "circle"); text(els.welcomeTitle, sectionId === "attendance-section" ? "Absensi Pembelajaran" : item.dataset.label === "Profile" ? "Profile Center" : item.dataset.label === "Jurnal Trade" ? "Trade Journal" : item.dataset.label === "Daily Bias" ? "Daily Bias" : "Performance Dashboard"); }
  });
